 ...
 function formvalidation_login() {
 	$('input').blur(function () {
 		...
 		if ($(this).hasClass('loginpassword')) {
 			if ($(this).val().length == '') {
 				$(this).siblings('span.error').text('kolom Password wajib diisi').fadeIn().parent('.form-group').addClass('hasError');
 			} else {
 				$(this).siblings('.error').text('').fadeOut().parent('.form-group').removeClass('hasError');
 			}
 		}
 			...
 			success: function (response) {
 				if (response.status == 'success') {
 					...
 					if (response.message.username) {
 						$('input.loginname').siblings('span.error').text(response.message.username).fadeIn().parent('.form-group').addClass('hasError');
 					}

 					if (response.message.password) {
 						$('input.loginpassword').siblings('span.error').text(response.message.password).fadeIn().parent('.form-group').addClass('hasError');
